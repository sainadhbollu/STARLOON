...
//import  { BleManager } BleManager2 from "react-native-ble-manager";
...
const Devices = (props) => {
  ...
  encodeData = (s) => {
    let len = s.length();
    let data = new byte[len / 2]();
    for (let i = 0; i < len; i += 2) {
      data[i / 2] = byte(parse.int(s.charAt(i)) + parse.int(s.charAt(i + 1)));
    }
    return data;
  };
  ...
  const scanForDevice = () => {
    if (!scanning) {
      ...
      //     if (error) {
      //       reject(`Error scanning for devices: ${error.message}`);
      //       return;
      //     }
      ...
      if (device) {
        console.error(`Could not find device with name Pheezee`);
        return;
      }
      ...
      //   const device = devices.find((device) => device.name === "Pheezee");
      //   if (!device) {
      //     console.error(`Could not find device with name Pheezee`);
      //     return;
      //   }
      ...
      //       // Retrieve the specified service and characteristic
      //       return BleManager.retrieveServices(device.id);
      //     })
      //     .then((services) => {
      //       const service = services.find(
      //         (service) =>
      //           service.uuid === "909a1400-9693-4920-96e6-893c0157fedd"
      //       );
      //       if (!service) {
      //         console.error(
      //           `Could not find service with UUID 909a1400-9693-4920-96e6-893c0157fedd`
      //         );
      //         throw new Error(
      //           `Could not find service with UUID 909a1400-9693-4920-96e6-893c0157fedd`
      //         );
      ...
      //       return BleManager.retrieveCharacteristics(service.uuid);
      //     })
      //     .then((characteristics) => {
      //       const characteristic = characteristics.find(
      //         (characteristic) =>
      //           characteristic.uuid === "909a1401-9693-4920-96e6-893c0157fedd"
      //       );
      //       if (!characteristic) {
      //         console.error(
      //           `Could not find characteristic with UUID 909a1401-9693-4920-96e6-893c0157fedd`
      //         );
      //         throw new Error(
      //           `Could not find characteristic with UUID 909a1401-9693-4920-96e6-893c0157fedd`
      //         );
      ...
      //         .then(() => {
      //           console.log(
      //             `Subscribed to notifications for characteristic ${characteristic.uuid}`
      //           );
      //         })
      //         .catch((error) =>
      //           console.error(
      ...
      //         .then((responseBytes) => {
      //           console.log(
      //             `Received ${responseBytes.length} bytes: ${responseBytes}`
      //           );
      //         })
      //         .catch((error) =>
      ...
      //     manager.scan(serviceUUIDs, null, (error, devices) => {
      //     .then(async (data) => {
      //     })
      //         .catch((error) => {
      ...
      //     console.log("Error starting scan:", "error");
      //     // if (error) {
      //     // console.error(`Error scanning for devices: ""`);
      //     // return;
      //     // }
      ...
      //     // const device = devices.find((device) => device.name === "Pheezee");
      //     // if (!device) {
      //     //   console.error(`Could not find device with name YOUR_DEVICE_NAME`);
      //     //   return;
      //     // }
      ...
      //     // BleManager.connect(device.id)
      //     //   .then(() => {
      //     //     // discover the specified service and characteristic
      //     //     return BleManager.retrieveServices(device.id);
      //     //   })
      //     //   .then((services) => {
      //     //     const service = services.find(
      ...
      //     //     );
      //     //     if (!service) {
      //     //       console.error(`Could not find service with UUID ${serviceUUID}`);
      //     //       throw new Error(
      //     //         `Could not find service with UUID ${serviceUUID}`
      //     //       );
      //     //     }
      //     //     return BleManager.retrieveCharacteristics(service.uuid);
      //     //   })
      ...
      //     //     if (!characteristic) {
      //     //       console.error(
      //     //         `Could not find characteristic with UUID ${characteristicUUID}`
      //     //       );
      //     //       throw new Error(
      //     //         `Could not find characteristic with UUID ${characteristicUUID}`
      //     //       );
      ...
      //     //       .then(() => {
      //     //         console.log(
      //     //           `Subscribed to notifications for characteristic ${characteristicUUID}`
      //     //         );
      //     //       })
      //     //       .catch((error) =>
      //     //         console.error(
      ...
      // manager.scanForDevice(serviceUUIDs, null, (error, device) => {
      ...
      // setTimeout(() => {
      //   if (manager && manager._scanEventSubscription) manager.stopScan();
  ...
  //       // manager
  //       //   .checkState()
  //       //   .then((state) => console.log(`current BLE state ='${state}'.`));
  ...
  //             );
  //             let write = await characteristic.writeWithResponse("0xAA01");
  //             console.log(
  //               "Writr & characteristics Value",
  //               write.value,
  //               characteristic.value
  //             );
  ...
  const readCharacteristics = async (device) => {
    //console.log("55555555555555555555555", data);
    ...
    //     service.uuid
    //   );
    //   console.log("READ CHARACTER LENGTH", characteristics.length);
    //   //let write = await characteristics.writeWithResponse(["0xAA02"]);
    //   // console.log("Write a  chara", write);
    //   for (let characteristic of characteristics) {
    ...
    //     // );
    //     // console.log("ZZZZZZZZZZZZZZZZZZZZZZZZZZZZZ", write);
    //   }
    // }

    // BleManager.readCharacteristics(
    //   device.id,
    //   "909a1400-9693-4920-96e6-893c0157fedd",
    //   "909a1401-9693-4920-96e6-893c0157fedd"
    // )
    ...
    // )
    //   .then((data) => {
    //     console.log("DEVICE INFO", device.id);
